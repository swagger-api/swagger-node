...
var TEST_DEPENDENCIES = {
  'z-schema': {'z-schema': '^3.12.0'},
  request: {request: '^2.58.0'},
  chai: {chai: '^3.0.0'},
  mocha: {mocha: '^2.2.5'},
  dotenv: {dotenv: '^1.2.0'}
}
...
function testGenerate(directory, options, cb) {
  ...
  findProjectFile(directory, null, function(err, projPath) {
    ...
    for (var dependency in TEST_DEPENDENCIES) {
      if (!projectJson.devDependencies.hasOwnProperty(dependency)) {
        lodash.extend(projectJson.devDependencies, TEST_DEPENDENCIES[dependency]);
        runInstall = true;
      }
    }

    if (!projectJson.hasOwnProperty('scripts')) {
      lodash.extend(projectJson, {scripts: {test: 'mocha test'}});

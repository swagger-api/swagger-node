...
var ASSERTIONTYPES = ['should', 'expect', 'assert'];
var TESTMODULES = ['request', 'supertest'];
...
function testGenerate(options, cb) {
  ...
  var desiredPaths = [];
  var testmodule = 'supertest';
  var assertionFormat = 'expect';
  var asynchronous = true;
  var separate = false;

  // parse swagger.yaml, get the json object
  var yaml = require('js-yaml');
  var string = fs.readFileSync('./api/swagger/swagger.yaml', {encoding: 'utf8'});
  var project = JSON.parse(string);
  ...
  if (options.testmodule) {
    ...
    if (!TESTMODULES[options.testmodule]) {
      return cb(new Error(util.format('Unknown type: %j. Valid types: %s', options.testmodule, Object.keys(TESTMODULES).join(', '))));
    }
  }

  // check if the assertion-format is one of the three
  if (options.assertionFormat) {
    assertionFormat = options.assertionFormat;
    if (!ASSERTIONTYPES[options.assertionFormat]) {
      return cb(new Error(util.format('Unknown type: %j. Valid types: %s', options.assertionFormat, Object.keys(ASSERTIONTYPES).join(', '))));
    }
  }

  // check if the asynchronous is only true or false
  if (options.asynchronous) {
    asynchronous = options.asynchronous;
    if (!ISTRUE[options.asynchronous]) {
      return cb(new Error(util.format('Unknown type: %j. Valid types: %s', options.asynchronous, Object.keys(ISTRUE).join(', '))));
    }
  }

  // check if the separate is only true or false
  if (options.separate) {
    separate = options.separate;
    if (!ISTRUE[options.separate]) {
      return cb(new Error(util.format('Unknown type: %j. Valid types: %s', options.separate, Object.keys(ISTRUE).join(', '))));
    }
  }
  ...
  if (config.separate) {
    for (var i = 0; i < result.length; i++) {
      ...
      var path = obj.name;
      var test = obj.test;
      var stringToWrite = path + '/n' + test;
      var filename = path.substring(1) + '.js';
      fs.writeFile('/test/'+filename, stringToWrite, function (err) {
        if (err) throw err;
        console.log('It\'s saved!');
      } )
    }
  } else {
    fs.writeFile('/test/test.js', result, function (err) {
      if (err) throw err;
      console.log('It\'s saved!');
    });
  }

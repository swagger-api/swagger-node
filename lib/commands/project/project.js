...
function testGenerate(pathname, options, cb) {
  ...
  if (options.type && !TYPE[options.type]) {
    return cb(new Error(util.format('Unknown type: %j. Valid types: %s', options.type, Object.keys(ASSERTIONTYPES).join(', '))));
  }
  ...
  var result = template.testGen(swagger, config, project);

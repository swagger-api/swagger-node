...
function test(directory, options, cb) {
  ...
  readProject(directory, options, function(err, project) {
    ...
    process.env.swagger_host = server_config[options.host].http_https + "://";
    ...
    readProject(directory, options, function(err, project) {
      ...
      swaggerSpec.validateSwagger(project.api.swagger, options, function(err) {
        spec.resolve(project.api.swagger, function(err, result) {
          ...
          var existingFiles = fs.readdirSync(path.join(directory, 'test/api/client', config.lang));
          var skipAll = false;
          ...
          async.filterSeries(finalResult, function(file, cb) {
            ...
            finalResult.forEach(function(file){
		            fs.outputFile(path.join(directory,'/test/api/client', config.lang, file.name), file.test, cb);
		            });
          });
        });
      });

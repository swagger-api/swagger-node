  ...
  if (allowedMethods.indexOf(currentMethod) > -1) {
    app[currentMethod](fullPath, function (req, res, next) {
      ...
      if (!self.canAccessResource(req, path, req.method)) {
        ...
        // validate the incoming parameters matches model / type definitions
        var validator = new Validation(req, root, api, spec);
        var validationErrors = validator.validate();
        if(validationErrors){
          res.send(JSON.stringify({
            'code': 400,
            'message' : 'The input did not validate successfully. ' + validationErrors.join('. ')
          }), 400);
        }
